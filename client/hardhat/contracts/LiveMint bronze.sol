...
//   event Start(uint auctionId, uint highestBid, uint256 timestamp);
//   event End(address[] winningBidders, uint auctionId, uint256 timestamp);
//   event Bid(address indexed sender, uint amount, uint auctionId, uint256 timestamp);
...
//   mapping(uint => uint) public auctionIdToTotalNFTs;
//   mapping(uint => mapping(address => address)) _nextBidders;
...
//     require(_nextBidders[auctionId][bidder] == address(0), "You must increase bid to add a bidder");
    
//     address index = _findIndex(bid);
//     fundsByBidder[auctionId][bidder] = bid;
//     _nextBidders[auctionId][bidder] = _nextBidders[auctionId][index];
//     _nextBidders[auctionId][index] = bidder;
//     listSize++;
//   }

//   function increaseBid(address bidder, uint256 bid) internal {
    
//     updateBid(bidder, fundsByBidder[auctionId][bidder] + bid);
//   }

//   function updateBid(address bidder, uint256 newBid) internal {
//     require(_nextBidders[auctionId][bidder] != address(0));
    
//     address prevBidder = _findPrevBidder(bidder);
//     address nextBidder = _nextBidders[auctionId][bidder];
//     if(_verifyIndex(prevBidder, newBid, nextBidder)){
//       fundsByBidder[auctionId][bidder] = newBid;
//     } else {
//       removeBidder(bidder);
...
//     address prevBidder = _findPrevBidder(bidder);
//     _nextBidders[auctionId][prevBidder] = _nextBidders[auctionId][bidder];
//     _nextBidders[auctionId][bidder] = address(0);
//     fundsByBidder[auctionId][bidder] = 0;
//     listSize--;
//   }
...
//     address currentAddress = _nextBidders[auctionId][GUARD];
//     for(uint256 i = 0; i < k; ++i) {
//       bidderLists[i] = currentAddress;
//       currentAddress = _nextBidders[auctionId][currentAddress];
//     }
//     return bidderLists;
//   }
...
//   //     bids[i] = fundsByBidder[auctionId][bidders[i]];
//   //   }
//   //   return bids;
//   // }
...
//     returns(bool)
//   {
//     return (prevBidder == GUARD || fundsByBidder[auctionId][prevBidder] >= newValue) && 
//            (nextBidder == GUARD || newValue > fundsByBidder[auctionId][nextBidder]);
//   }

//   function _findIndex(uint256 newValue) internal view returns(address) {
//     address candidateAddress = GUARD;
//     while(true) {
//       if(_verifyIndex(candidateAddress, newValue, _nextBidders[auctionId][candidateAddress]))
//         return candidateAddress;
//       candidateAddress = _nextBidders[auctionId][candidateAddress];
//     }
//   }

//   function _isPrevBidder(address bidder, address prevBidder) internal view returns(bool) {
//     return _nextBidders[auctionId][prevBidder] == bidder;
//   }

//   function _findPrevBidder(address bidder) internal view returns(address) {
//     address currentAddress = GUARD;
//     while(_nextBidders[auctionId][currentAddress] != GUARD) {
...
//   function getBid(uint currentAuctionId) external view returns (uint256) {
//       return fundsByBidder[currentAuctionId][msg.sender];
//   }

//   function getLowestBid(uint currentAuctionId) external view returns (uint256) {
//       address[] memory bidders = getTop(auctionIdToTotalNFTs[currentAuctionId]);
//       uint topLength = bidders.length;
//       return fundsByBidder[currentAuctionId][bidders[topLength-1]];
//   }

//   function getHighestBid(uint currentAuctionId) external view returns (uint256) {
//       address[] memory bidders = getTop(auctionIdToTotalNFTs[currentAuctionId]);
//       return fundsByBidder[currentAuctionId][bidders[0]];
//   }
...
//         auctionId = _auctionsCreated.current();
//         auctionIdToTotalNFTs[auctionId] = nftsToMint;
//         _nextBidders[auctionId][GUARD] = GUARD;
...
//         emit Start(auctionId, floorBid, block.timestamp);
...
//         require(msg.value >= floorBid, "Must Bid Higher than or equal to Floor");
//         address lastBidder;
        
//         if (auctionIdToTotalNFTs[auctionId] <= listSize) {
//           lastBidder = getTop(auctionIdToTotalNFTs[auctionId])[auctionIdToTotalNFTs[auctionId]-1];
//         }

//         if (lastBidder != address(0)) {
//             require(msg.value+fundsByBidder[auctionId][msg.sender] > fundsByBidder[auctionId][lastBidder], "Must bid more than the last highest bidder");
//         }

//         if (_nextBidders[auctionId][msg.sender] == address(0)) {
//           addBidder(msg.sender, msg.value);
//         } else {
//           increaseBid(msg.sender, msg.value);
//         }

//         emit Bid(msg.sender, fundsByBidder[auctionId][msg.sender], auctionId, block.timestamp);
//   }

//    function withdraw() external payable nonReentrant {
//         if (auctionIdToTotalNFTs[auctionId] <= listSize) {
//           address[] memory topBidders = getTop(auctionIdToTotalNFTs[auctionId]);
//           for (uint i = 0; i < auctionIdToTotalNFTs[auctionId]; i++) {
...
//         uint bal = fundsByBidder[auctionId][msg.sender];
...
//     uint nftsMinted = auctionIdToTotalNFTs[auctionId];
...
//             payable(seller).transfer(fundsByBidder[auctionId][winningBidders[i]]);
...
//     emit End(winningBidders, auctionId, block.timestamp);
//     auctionId = _auctionsCreated.current();
//   }
