...
contract LiveMintFactoryMultiple is ERC721URIStorage, Ownable, ERC2981, ReentrancyGuard {
    ...
    modifier onlyValidRoyaltyBPS(uint96 bps) {
        require(bps < _MAX_BPS);
        _;
    }
    ...
    ) ERC721(name, symbol) onlyValidRoyaltyBPS(_royaltyValue) {
        theArtist = artist;
...
contract LiveMintAuctionMultiple is ReentrancyGuard {
  using Counters for Counters.Counter;
...
////////////////////// GETTERS //////////////////////////////
  ...
  function getCurrentItem() external view returns (uint256) {
      LiveMintFactoryMultiple mintContract = LiveMintFactoryMultiple(nftContract); 
...
////////////////////////////////////////////////////////////////
  ...
  function end(string[] calldata tokenURI) external nonReentrant{
        ...
        for (uint i = 0; i < tokenURI.length; i++) {
            if (winningBidders[i] != address(0)) {
                ...
                uint tokenId = mintContract.mint(tokenURI[i]);
