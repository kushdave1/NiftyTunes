...
function BidCard({auctionAddress, signerAddress, responsive, editionsPerAuction}) {
  ...
  const updateRemainingTime = async(countdown) => {
      ...
      setRemainingTime({
        seconds: PadWithZeros(secondsLeft.toString()),
        minutes: PadWithZeros(minutesLeft.toString()),
        hours: PadWithZeros(hoursLeft.toString())
      })
  ...
  return (
    <>
        ...
        <Card className="bg-light shadow-sm"
              style={{ width: '24rem', height: '17rem', borderRadius:'.50rem',  fontSize: 8, overflow: "hidden", zIndex: "1"}} >
